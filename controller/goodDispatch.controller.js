...
export const ViewWarehouseByOrder = async (req, res, next) => {
    try {
        const order = await CreateOrder.find({ "orderItems.warehouse": req.params.id, status: "Dispatch" }).populate({ path: "orderItems.productId", model: "product" }).populate({ path: "partyId", model: "customer" }).populate({ path: "userId", model: "user" })
        if (!order) {
