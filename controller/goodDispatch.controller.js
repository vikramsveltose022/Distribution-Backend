...
export const viewOrderForDeliveryBoy = async (req, res, next) => {
    try {
        ...
        // }
        let goodDispatch = await CreateOrder.find({ database: database, userId: userId }).sort({ sortorder: -1 }).populate({ path: "orderItems.productId", model: "product" }).populate({ path: "userId", model: "user" }).populate({ path: "partyId", model: "customer" }).populate({ path: "orderId", model: "createOrder" }).populate({ path: "warehouseId", model: "warehouse" })
